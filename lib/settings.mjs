...
const name = "client-ui-settings-remote";
...
function apply(ctx) {
	...
	ctx.effect(() => connection.rpc.handle("/remote-admin", (endpoint, payload) => dispatch(ctx, endpoint, payload), { authority: "loopback-peer" }), "ui-settings-remote: Host-local administration channel");
}
	...
	try {
		...
		switch (endpoint) {
			...
			case "discovery/set":
				...
				return success(snapshot(admin));
			default: return badRequest("Unknown Remote administration endpoint");
		}
	} catch (error) {
...
function identifierPayload(payload, key) {
	if (!isRecord(payload) || Reflect.ownKeys(payload).length !== 1 || !Object.hasOwn(payload, key) || typeof payload[key] !== "string" || !/^[0-9a-f]{32}$/iu.test(payload[key])) throw new InvalidAdminRequest(`${key} must be a 16-byte hexadecimal identifier`);
	return payload[key];
}
function profilePayload(payload) {
	if (!isRecord(payload) || Reflect.ownKeys(payload).length !== 1 || !Object.hasOwn(payload, "profile") || payload.profile !== "read-only" && payload.profile !== "approval-reviewer" && payload.profile !== "session-control" && payload.profile !== "session-operator" && payload.profile !== "session-supervisor" && payload.profile !== "host-supervisor") throw new InvalidAdminRequest("profile must be a supported Remote pairing profile");
	return payload.profile;
}
function enabledPayload(payload) {
	if (!isRecord(payload) || Reflect.ownKeys(payload).length !== 1 || !Object.hasOwn(payload, "enabled") || typeof payload.enabled !== "boolean") throw new InvalidAdminRequest("enabled must be a boolean");
